...
(with-eval-after-load 'helm-etags+
  (add-hook 'helm-etags+-before-jump-hook '(lambda()(bm-bookmark-add nil nil t)))
  (add-hook 'helm-etags+-after-jump-hook '(lambda()(bm-bookmark-remove)))
  )
...
(defun my-helm-gtags-mode-hook()
  ...
  (local-set-key (kbd "C-[ [ a c") 'helm-gtags-complete)   ; iterm map to ctrl-return
  )
;; key bindings
