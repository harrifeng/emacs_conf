;;; joseph-python-auto-import.el --- Description
...
(defun --save-result (symbol result)
  (if (not (string-prefix-p "from ." result))
      (progn
        (puthash symbol result --symbol-cache)
        (--dump-symbol-cache)))
  result)

(defvar python-auto-import-cache-filename "~/.emacs.d/cache/python-auto-import-cache")
...
(defun python-autoimport--is-first-component-enough (symbol)
  (or
   (string-prefix-p "os.path." symbol)
   ))
...
(provide 'joseph-python-auto-import)

;; Local Variables:
;; coding: utf-8
;; End:

;;; joseph-python-auto-import.el ends here
